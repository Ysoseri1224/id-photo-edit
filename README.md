...
> 一款离线优先的证件照处理工具。上传照片，裁剪人像，自动换底色，导出符合常见报名/签证/护照尺寸要求的证件照。

<p align="center">
  <img src="./assets/readme-cover.svg" alt="证件照处理工具封面" width="100%" />
</p>
...
它的目标不是做一个“图片编辑器”，而是把证件照这件高频但琐碎的事做成一条顺手的流程：
...
整个项目采用 `Electron + React + FastAPI` 架构，前端负责交互，后端负责抠图与图像处理。默认支持本地运行，适合继续扩展成完整桌面应用。

## 为什么做这个

很多证件照工具要么太重，要么太像在线表单，要么流程不透明。
...
- 缺模型时给明确引导，而不是直接抛一个难懂错误

## 核心功能

- 常见证件照规格预设
  支持一寸、二寸、中国护照、美国签证、申根签证、高考报名、考研报名、驾照、简历照和自定义规格。
...
- 裁剪、参数设置、结果预览都在同一界面完成
- 下载模型时展示来源、进度、速度和预计剩余时间
- 网络或后端未启动时，前端会给更明确的中文错误提示

如果你想把它继续做成可分发的桌面工具，这个仓库已经具备一个比较干净的起点。
...
- 需要频繁处理报名照/签证照的人
- 想要本地离线处理照片的人
- 想基于 Electron + FastAPI 做桌面 AI 工具原型的人
- 想继续扩展证件照规格、导出规则或模型管理能力的开发者
...
因为它依赖 `BiRefNet` 本地模型。首次使用前需要在“模型管理”里下载。
...
这个命令会同时启动前端和后端。只启动前端时，API 请求会失败。
...
首个公开版本：[`v1.0.0`](https://github.com/Ysoseri1224/id-photo-edit/releases/tag/v1.0.0)
